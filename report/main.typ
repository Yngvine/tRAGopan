...
We expose a connection to that Ollama service via Tailscale. The frontend GUI connects to the project backend, which routes intent through the remote GPU while simultaneously executing WFS calls to IDENA. As our "remote brain," the system initially utilized the Qwen3.5-35B model @qwen2024. However, because this model severely constrained the Virtual Machine's storage capacity and introduced high inference latency, we ultimately migrated to Llama3-7B @llama3_2024, which provided a significant improvement in response speed (x5) while maintaining the necessary reasoning capabilities for tool orchestration.
...
#figure(
  ...
  caption: [Categorization of the top 20 failure cases per model. Mode (1): Spurious Spatial Cross-talk; Mode (2): Pagination & Density Limits; Mode (3): Model Hallucination. Whilst Llama3-7B providing necessary operational speed improvements, we can see the bigger size of Qwen3.5-35B working towards improved performance.]
) <tab:error_comparison>

The failures categorized primarily into three root causes, as detailed in @tab:error_comparison:

1. *Spurious Spatial Cross-talk*: Complex queries involving negation or exclusion ("Show parcels EXCEPT in Zone A") confused the WFS query builder. While Qwen3.5-35B navigated these logical traps somewhat better (2 cases), Llama3-7B's reduced parameter count doubled these errors (4 cases), highlighting a weakened native understanding of topological negation. The model frequently defaulted to an unconstrained query (e.g., "Show me all toponyms in Navarre EXCEPT those in Pamplona" -> LLM fails to apply the spatial exclusion filter, resulting in a complete dump of all toponyms).
2. *Pagination & Density Limits*: This failure mode remained consistently high across both models (7 vs. 8 cases), demonstrating that this is primarily an infrastructural limitation rather than a purely linguistic one. For highly dense layers (like wide-area toponymy), the WFS limit of 2000 features truncated results before the system could retrieve the entire dataset. In both models, the orchestrator failed to recognize the server-side pagination cap and confidently hallucinated completeness to the user.
3. *Model Hallucination*: This category saw the most drastic degradation when migrating to the smaller model, quadrupling from 2 cases in Qwen3.5-35B to 8 in Llama3-7B. The 7B parameter model frequently broke the deterministic orchestration by generating critically flawed tool parameters, selecting incorrect layers, or hallucinating geographic falsehoods (e.g., "Show me toponyms in Villava" -> LLM abandons the query because it incorrectly asserts that "Municipality 'Villava' belongs to the province of Bizkaia").



= Conclusion and Future Work

IDENA-SVP successfully demonstrates an alternative to traditional, high-friction GIS interfaces. By bridging LLM routing with IDENA WFS infrastructure, we out-performed manual GIS baseline workflows in both execution time and accessibility for non-expert users. Crucially, this agentic approach almost entirely abstracts away the steep "knowledge stack" typically required in manual GIS. Users no longer need to understand Coordinate Reference Systems, OGC service differences, or complex WFS query formatting. The only remaining requisite is a general awareness of the types of spatial features available; yet, even this barrier is mitigated by the LLM's capacity to interactively explain layer contents and suggest valid capabilities dynamically. However, our failure analysis also reveals that while simple geometric intersection is highly reliable via Agentic LLMs, resolving topological negation and parsing overlapping feature bounds requires the usage of either a more capable model or better integration strategies.
